...
calls. Missing plotted points are retained as ``success=False`` CSV rows,
typically with ``error_type=OutOfMemoryError`` or ``SkippedAfterOOM``.
```
...
(i.e. ``compute_charge_gradients=False``). Charge-gradient variants
(``pme_cg`` / ``ewald_cg``) are benchmarked for both backends and retained
in the CSVs, but omitted from the comparison panels so the plotted series
stay focused on the base force/energy API.
...
CSV rows include `time_real_us_per_atom` and `time_reciprocal_us_per_atom` so
real-space and reciprocal-space timing can be audited separately from the total
per-atom timing.
