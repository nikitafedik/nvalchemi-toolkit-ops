...
Usage:
    ...
    python benchmark_suite.py --count --benchmark all
...
def main():
    ...
    if not getattr(args, "count_plan", False):
        import torch
    ...
    if plan_only:
        ...
        label = "COUNT" if getattr(args, "count_plan", False) else "DRY RUN"
        print(f"\n{label} COMPLETE: {total} planned row(s)")
...
def _generate_plots(results_dir, plots=None):
    ...
    csvs = [path for path in all_csvs if path.name.startswith(SUITE_CSV_PREFIXES)]
    if not csvs:
    ...
    if all_panels:
        for csv in csvs:
            attempted += 1
            try:
                if detect_and_plot(csv, three_panel_dir):
                    succeeded += 1
                else:
                    msg = f"3-panel {csv.name}: no successful data"
    ...
    for csv in csvs:
        for panel in panels:
            ...
            try:
                ...
                plot_errors.append(msg)
                print(f"  ERROR ({msg})")
            if csv.name.startswith("nl-"):
                for cutoff in NL_DOC_CUTOFFS:
                    attempted += 1
                    try:
                        cutoff_label = f"{cutoff:g}A"
                        out = (
                            single_dir / f"{csv.stem}-cutoff-{cutoff_label}-{panel}.png"
                        )
                        if plot_single_panel(
                            ...
                            msg = (
                                f"single {csv.stem}-cutoff-{cutoff_label}-{panel}: "
                                "no successful data"
                            )
                            plot_errors.append(msg)
                            print(f"  ERROR ({msg})")
                    except Exception as e:
                        msg = f"single {csv.stem}-cutoff-{cutoff:g}A-{panel}: {e}"
                        plot_errors.append(msg)
                        print(f"  ERROR ({msg})")
