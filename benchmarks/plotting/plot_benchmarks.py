...
def load_csv(filepath: str | Path) -> list[dict[str, Any]]:
    ...
    Filters out failed rows (success=False) since timing and throughput fields
    are missing/NaN and should not be plotted as measurements.
    """
    results = []
        ...
        for row in reader:
            for key in row:
                try:
                    ...
                    pass
            # Skip failed rows; NaN timings are policy/failure metadata, not data.
...
def plot_single_panel(
    ...
    *,
    ...
    # Dispatch to the right plotting logic. Failed or OOM rows remain in
    # the CSV with success=False, and load_csv filters them out so plots
    # show a missing point rather than a zero-valued measurement.
    if module == "nl":
...
def _nl_method_family(method: str) -> str:
    """Map legacy and canonical NL method names to a plotted strategy family."""
...
def _should_merge_memory(data: list[dict[str, Any]]) -> bool:
    """Return whether NL memory methods should be merged.

    The suite plots keep NL methods separate because scalar/tile and
    atom-/pair-centric paths are user-facing benchmark choices.
    """
    return False
...
def _nl_mergeable_memory_group(method: str) -> str:
    """Return the old memory-merge group name for unreachable legacy code paths."""
    family = _nl_method_family(method)
    if family == "naive":
        return "naive"
    if family == "cell_list":
...
def _group_nl_by_cutoff_merged(
    ...
    for r in data:
        if _nl_mergeable_memory_group(r["method"]) == "cell":
...
def _render_nl_batch(
    ...
    if merge_methods:
        ...
        for r in data_15:
            family = _nl_mergeable_memory_group(r["method"])
            if family == "cell":
        ...
        if not grouped:  # fallback to naive
            for r in data_15:
                if _nl_mergeable_memory_group(r["method"]) == "naive":
                    grouped[r["atoms_per_system"]].append(r)
...
def plot_comparison_panel(
    ...
    for r in data:
        if merge_nl_memory:
            family = _nl_mergeable_memory_group(r.get("method", ""))
